// Adapted from bevy_scene_macros/src/bsn/parse.rs (bevyengine/bevy main branch)
// Changed import path from crate::bsn::types to crate::bsn_ast
...
impl PathType {
    fn new(path: &Path) -> PathType {
        ...
        if let Some(last_segment) = iter.next() {
            ...
            let last_ident_first_char = last_string.chars().next().unwrap();
